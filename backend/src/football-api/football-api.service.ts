...
export class FootballApiService {
  ...
  /**
   * Create test matches for development/testing
   */
  async createTestMatches(): Promise<Match[]> {
    const testMatchesData = [
      {
        externalId: 'test-1',
        homeTeam: 'Manchester United',
        awayTeam: 'Liverpool',
        matchTime: Math.floor(Date.now() / 1000) + 86400 * 2, // 2 days from now
        status: MatchStatus.SCHEDULED,
      },
      {
        externalId: 'test-2',
        homeTeam: 'Arsenal',
        awayTeam: 'Chelsea',
        matchTime: Math.floor(Date.now() / 1000) + 86400 * 3, // 3 days from now
        status: MatchStatus.SCHEDULED,
      },
      {
        externalId: 'test-3',
        ...
        awayTeam: 'Tottenham',
        matchTime: Math.floor(Date.now() / 1000) + 86400 * 4, // 4 days from now
        status: MatchStatus.SCHEDULED,
      },
      {
        externalId: 'test-4',
        homeTeam: 'Newcastle',
        awayTeam: 'Brighton',
        matchTime: Math.floor(Date.now() / 1000) + 86400 * 5, // 5 days from now
        status: MatchStatus.SCHEDULED,
      },
      {
        externalId: 'test-5',
        ...
        awayTeam: 'West Ham',
        matchTime: Math.floor(Date.now() / 1000) + 86400 * 6, // 6 days from now
        status: MatchStatus.SCHEDULED,
      },
      {
        externalId: 'test-6',
        ...
        awayTeam: 'Barcelona',
        matchTime: Math.floor(Date.now() / 1000) + 86400 * 7, // 7 days from now
        status: MatchStatus.SCHEDULED,
      },
      {
        externalId: 'test-7',
        ...
        matchTime: Math.floor(Date.now() / 1000) + 86400 * 8, // 8 days from now
        status: MatchStatus.SCHEDULED,
      },
      {
        externalId: 'test-8',
        ...
        matchTime: Math.floor(Date.now() / 1000) + 86400 * 9, // 9 days from now
        status: MatchStatus.SCHEDULED,
      },
    ];

    const matches: Match[] = [];
    for (const matchData of testMatchesData) {
    ...
    this.logger.log(`Created ${matches.length} test matches`);
    return matches;
  }
}
